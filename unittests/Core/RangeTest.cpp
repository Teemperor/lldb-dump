...
TEST(RangeTest, LessThan) {
  ...
  EXPECT_FALSE(RangeT(10, 21) < r);

  // Start before, but ends before.
  EXPECT_FALSE(r < RangeT(9, 20));
  EXPECT_TRUE(RangeT(9, 20) < r);

  // Start before, but equal size.
  EXPECT_FALSE(r < RangeT(9, 21));
  EXPECT_TRUE(RangeT(9, 21) < r);

  // Start before, but bigger size.
  EXPECT_FALSE(r < RangeT(9, 22));
  EXPECT_TRUE(RangeT(9, 22) < r);

  // Start after, but ends before.
  EXPECT_TRUE(r < RangeT(11, 18));
  EXPECT_FALSE(RangeT(11, 18) < r);

  // Start after, but equal size.
  EXPECT_TRUE(r < RangeT(11, 19));
  EXPECT_FALSE(RangeT(11, 19) < r);

  // Start after, but bigger size.
  EXPECT_TRUE(r < RangeT(11, 20));
  EXPECT_FALSE(RangeT(11, 20) < r);
}
