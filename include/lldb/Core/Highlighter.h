...
public:
  ...
  /// Makes the Highlighter decide if it wants to color the given source code.
  /// \param language
  ///     The language that the caller thinks the source code was given in.
  /// \param path
  ///     The path to the file the source code is from. Used as a fallback when
  ///     the user can't provide a language.
  /// \return
  ///     True if the Highlighter is sure he can highlight the file; otherwise
  ///     returns false.
  virtual bool ShouldHighlightFile(lldb::LanguageType language,
                                   llvm::StringRef path) const = 0;
};

/// A default highlighter that does nothing. Used as a fallback highlighter.
...
public:
  ...
  const Highlighter &getHighlighterFor(lldb::LanguageType language,
