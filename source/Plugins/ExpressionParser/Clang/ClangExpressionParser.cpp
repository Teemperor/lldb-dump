...
ClangExpressionParser::ParseInternal(DiagnosticManager &diagnostic_manager,
  ...
  if (completion_consumer) {
    ...
    // Columns start at 1, but code completion positions are indexed from 0,
    // so we need to add 1 to the column here.
    ++completion_column;
