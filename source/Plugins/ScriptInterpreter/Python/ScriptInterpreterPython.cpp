...
bool ScriptInterpreterPython::ExecuteOneLine(llvm::StringRef command, CommandReturnObject *result,
...
bool ScriptInterpreterPython::ExecuteOneLineWithReturn(llvm::StringRef in_string, ScriptInterpreter::ScriptReturnType return_type,
  ...
  { // scope for PythonInputReaderManager
    ...
    py_return.Reset(
        PyRefType::Owned,
        PyRun_String(as_string.c_str(), Py_eval_input, globals.get(), locals.get()));
    if (!py_return.IsValid()) {
      ...
      py_return.Reset(PyRefType::Owned,
                      PyRun_String(as_string.c_str(), Py_single_input, globals.get(),
                                   locals.get()));
    }
  }
...
bool ScriptInterpreterPython::RunScriptBasedCommand(const char *impl_function, llvm::StringRef args,
...

  {
    ...
    ret_val =
        g_swig_call_command(impl_function, m_dictionary_name.c_str(),
                            debugger_sp, args_str.c_str(), cmd_retobj, exe_ctx_ref_sp);
  }
